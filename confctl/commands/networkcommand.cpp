...
{   
    ...
    actions["ensure"] = [this, &cmd_tokens](){
        ...
        if (cmd_tokens.size() < 4 || allowed_states.end() == std::find(allowed_states.begin(), allowed_states.end(), cmd_tokens[3]))
            throw std::invalid_argument(string_format("network_cmd: [ensure] require 'on or off' state for net [%s]", this->net_name.c_str() ));
        this->default_val = cmd_tokens[3];
    };
...
{   
    ...
    actions["default"] = [&]() {
        ...
        {
            std::cout << "[E][network default] you have both enabled and disabled files for network [" << this->net_name << "], skip this step"<<std::endl;
            return false; 
        }
        ...
        if (fs::exists(template_copy_dest)) {
            std::cout << "[E][network default " << this->default_val<<"] Network [" << this->net_name << "] already "<< this->default_val << ", skip this step" <<std::endl;
            return true;
        }        
        ...
        std::cout << "[E][network default] Set-up net [" << this->net_name << "] as ["<< (this->default_val == "on" ? "ENABLED" : "DISABLED")
    ...
    actions["ensure"] = [&]() {
        ...
        {
            std::cout << "[E][network ensure on]: Network [" << this->net_name << "] already enabled"<<std::endl;
            return false;
        }
    
        if (this->default_val == "off" && !net_enabled && fs::exists(config_path(this->net_name, CFG_GENERAL, CFG_OFF)))
        {
            std::cout << "[E][network ensure on]: Network [" << this->net_name << "] already disabled"<<std::endl;
            return false;
        }
        ...
        if (fs::exists(config_path(this->net_name, CFG_GENERAL, oposite_state))){
            ...
            std::cout << "[E][network ensure " << this->default_val << "] move file from " 
            ...
            std::cout << "[E][network ensure "<<this->default_val << "] copy file from " 
